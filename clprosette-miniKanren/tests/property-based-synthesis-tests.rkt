...
(printf "property-based-synthesis-tests.rkt\n")
...
(test "synthesize-triple-by-property-1b"
  (run 1 (q)
    (fresh (f x x-tripled e)
      ...
      (rosette-typeo x-tripled r/@integer?)
      (rosette-asserto `(,r/@= ,x (,r/@* 3 ,x-tripled)))
      (evalo `(,f ,x) x-tripled)))
  '(((lambda (x) 0) 0 0)))
...
(test "evalo-fac-odd-n-1"
  (run 1 (q)
    (fresh (x y z)
      ...
      (rosette-asserto `(,r/@<= 0 ,y))
      (rosette-asserto `(,r/@<= 0 ,x))
      (rosette-asserto `(,r/@<= 0 ,z))
      ;; assert z is odd
      (rosette-asserto `(,r/@= ,z (,r/@+ (,r/@* 2 ,y) 1)))
      (evalo `(letrec ((fac
                        (lambda (n)
                          (if (< n 0) #f
                              (if (= n 0) 1
